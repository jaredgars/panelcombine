...
use `temp`num'', clear
	local j = `columncount' - 1 
	local k = substr("cccccccc",1,`j')
	if `num'==1 { //process first panel -- clip bottom
	replace v1 = "\begin{tabular}{p{4.5cm}*1{`k'}}" if v1=="\begin{tabular}{l*{`j'}{c}}" 
	...
	replace v1 = "\multicolumn{`columncount'}{l}{ } \\" if v1=="\hline" & _n>4
	...
	replace v1 = "" if strpos(v1,"Lee")>0  
	replace v1 = "\multicolumn{`columncount'}{l}{ } \\" if v1=="\hline" 
	replace v1 = "\hline \multicolumn{`columncount'}{l}{\linebreak \textbf{\textit{Panel `panellabel': `panel`num'title'}}} \\" if _n==1
	}
